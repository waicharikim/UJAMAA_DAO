...
export async function seedOnboardingFlows() {
  ...
  const citizenFlow = await prisma.onboardingFlow.upsert({
    ...
    create: {
      ...
      description:
        'Basic onboarding for citizens. Soft gating for basic features.',
      type: 'CITIZEN',
      version: 1,
      gatingMode: 'HYBRID',
      active: true,
  ...
  const creatorFlow = await prisma.onboardingFlow.upsert({
    ...
    create: {
      ...
      description:
        'Onboarding for creators: profile, industries, wallet, listings setup.',
      type: 'CREATOR',
      version: 1,
      gatingMode: 'HYBRID',
      active: true,
      steps: {
        create: [
          ...
          },
          {
            ...
            actionConfig: {
              requiredFields: ['name', 'industryId', 'avatarUrl'],
            },
  ...
  const groupAdminFlow = await prisma.onboardingFlow.upsert({
    ...
    create: {
      ...
      description:
        'Strict onboarding for group admins (treasury, verification, multi-sig).',
      type: 'GROUP_ADMIN',
      version: 1,
      gatingMode: 'STRICT',
      active: true,
        ...
        create: [
          {
            ...
            actionType: 'COMPLETE_PROFILE',
            actionConfig: {
              requiredFields: ['name', 'phoneNumber', 'walletAddress'],
            },
            successCriteria: { requiredFieldsCompleted: true },
            skippable: false,
          },
  ...
  const verifierFlow = await prisma.onboardingFlow.upsert({
    ...
    create: {
      ...
      description:
        'Strict onboarding for milestone/verifiers and experts (ID docs).',
      type: 'VERIFIER',
      version: 1,
      gatingMode: 'STRICT',
      active: true,
      steps: {
        create: [
          ...
          },
          {
            ...
            actionConfig: {
              requiredDocumentTypes: ['IDENTITY', 'CERTIFICATION'],
            },
  ...
  await prisma.roleOnboardingRequirement.upsert({
    where: {
      roleName_flowId: { roleName: 'group:admin', flowId: groupAdminFlow.id },
    },
    update: {},
    create: {
      id: uuidv4(),
