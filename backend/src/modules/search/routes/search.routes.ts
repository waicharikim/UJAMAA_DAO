...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const [users, proposals, modules, groups] = await Promise.all([
    prisma.user.findMany({
      ...
      select: { id: true, name: true, email: true, avatarUrl: true, verificationLevel: true },
      take: limit,
    }),
    ...
    prisma.proposal.findMany({
      ...
      select: { id: true, title: true, status: true, proposalType: true, createdAt: true },
    ...
    prisma.group.findMany({
      ...
      select: { id: true, name: true, memberCount: true, locationScope: true },
      take: limit,
    }),
  ]);
