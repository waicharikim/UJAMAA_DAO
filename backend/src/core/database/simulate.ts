...
async function phase2Groups(homeWardId: string) {
  ...
  const specs = [
    { creator: 'njeri', name: 'Kayole Maendeleo SACCO', type: 'SAVINGS_CREDIT',
      desc: 'A neighbourhood savings & credit cooperative — pool monthly, lend locally.' },
    { creator: 'david', name: 'Kayole Youth Network', type: 'YOUTH_ORGANIZATION',
      desc: 'Youth-led organising: skills, hustles, and a voice in ward decisions.' },
    { creator: 'grace', name: 'Ward Transparency Watch', type: 'TRANSPARENCY_WATCHDOG',
...
async function phase3Governance(group: { id: string; name: string }) {
  ...
  const proposals: {
    title: string; description: string; fundingAmountKes: number;
    creator: { id: string }; target: Target;
  }[] = [
    { title: 'Buy a shared 10,000L water tank', creator: joseph, fundingAmountKes: 85000,
      description: 'Install a communal water tank at the estate gate so households stop paying vendors per jerrican. Cheaper water, less queuing.', target: 'DRAFT' },
    { title: 'Hire an estate night security guard', creator: aisha, fundingAmountKes: 144000,
      description: 'Pool funds to hire a vetted night guard for the estate after the recent break-ins. Twelve-month contract, reviewed quarterly.', target: 'APPROVED' },
    { title: 'Fund a youth coding bootcamp', creator: peter, fundingAmountKes: 120000,
      description: 'A three-month coding bootcamp for 20 youth, run with the local IT hub. Laptops shared, trainers paid from the fund.', target: 'VOTING' },
    { title: 'Install solar street lights on Mtaa Lane', creator: leader, fundingAmountKes: 96000,
      description: 'Six solar street lights along the dark stretch of Mtaa Lane to improve safety for women and traders at night.', target: 'VOTING' },
    { title: 'Repair the estate access road', creator: joseph, fundingAmountKes: 210000,
      description: 'Grade and murram the access road before the rains. Quotes attached; work overseen by the projects committee.', target: 'EXECUTED' },
    { title: 'Buy branded SACCO merchandise', creator: aisha, fundingAmountKes: 40000,
      description: 'Branded T-shirts and caps for members. Nice-to-have, not essential right now.', target: 'REJECTED' },
  ];

  const members = await communityMembers();
  const statusOf = async (id: string) =>
    (await prisma.proposal.findUnique({ where: { id }, select: { status: true } }))!.status;

  for (const p of proposals) {
    try {
      // find-or-create (resumable — drives an existing proposal toward its target)
      let row = await prisma.proposal.findFirst({
        where: { title: p.title, groupId: group.id }, select: { id: true },
      });
      if (!row) {
        const created = await proposalService.createProposal(p.creator.id, {
          groupId: group.id, title: p.title, description: p.description,
          fundingAmountKes: p.fundingAmountKes, proposalScope: 'GROUP',
        });
        row = { id: (created as any).id as string };
      }
      const id = row.id;

      if (p.target === 'DRAFT') { console.log(`   ✓ DRAFT: ${p.title}`); continue; }
      ...
      if ((await statusOf(id)) === 'DRAFT') {
        await proposalService.reviewProposal(
          leader.id, id, { decision: 'APPROVE', note: 'Approved for a community vote.' }, [],
        );
        await annotate(members, id, p.description).catch(() => {});
      }
      if (p.target === 'APPROVED') { console.log(`   ✓ APPROVED: ${p.title}`); continue; }
      ...
      if ((await statusOf(id)) === 'VOTING') {
        const have = await prisma.groupMemberVote.count({ where: { proposalId: id } });
        if (have === 0) {
          let n = 0;
          for (let i = 0; i < members.length; i++) {
            const m = members[i];
            const option = wantPass
              ? (i % 6 === 0 ? VoteOption.NO : i % 7 === 0 ? VoteOption.ABSTAIN : VoteOption.YES)
              : (i % 5 === 0 ? VoteOption.YES : i % 6 === 0 ? VoteOption.ABSTAIN : VoteOption.NO);
            if (await proposalService
              .castVote(m.id, { proposalId: id, option }, m.primaryWardId ?? undefined)
              .then(() => true).catch(() => false)) n++;
          }
          console.log(`     ${p.title}: ${n} votes cast`);
        }
      }
      if (p.target === 'VOTING') { console.log(`   ✓ VOTING: ${p.title}`); continue; }

      // EXECUTED / REJECTED → tally (outcome decided by the vote skew)
      if ((await statusOf(id)) === 'VOTING') {
        await proposalService.tallyVotes(id, leader.id, []).catch((e) =>
          console.warn(`     tally: ${(e as Error).message}`));
      }
...
async function annotate(
  ...
  const picks = [
    { sub: description.slice(0, Math.min(24, description.length)), comment: 'Strongly support this — long overdue.' },
    { sub: description.slice(Math.max(0, description.length - 24)), comment: 'Can we see the quotes before voting?' },
  ];
