...
// Runtime type guards
export function isValidSystemRole(
  role: string
): role is (typeof SystemRoles)[SystemRole] {
  return Object.values(SystemRoles).includes(
    role as (typeof SystemRoles)[SystemRole]
  );
}
...
export type AssignmentMethodType =
  (typeof AssignmentMethod)[keyof typeof AssignmentMethod];
...
export const SystemRoleAssignment: Partial<
  Record<SystemRole, AssignmentMethodType>
> = {
