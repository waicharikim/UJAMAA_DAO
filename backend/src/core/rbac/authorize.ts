...
const isValidScope = (scope: string): boolean => /^[a-z]{2,20}(:[0-9a-f-]{8,36})?$/i.test(scope);
 ...
 */
const hasRole = (userRoles: string[] = [], role: string, scope?: Scope): boolean => {
 ...
 */
const requireSystemRole = (role: string) => (req: AuthRequest): void => {
 ...
 */
const requireGroupRole = (role: string, scopePrefix: string) => (req: AuthRequest): void => {
  ...
  const granted = hasRole(req.user?.roles, role, scopePrefix) || 
 ...
 */
const requireOwnershipOrAdmin = (resourceUserId: string, adminRole = "ADMIN") => (req: AuthRequest): void => {
  const userId = req.user?.userId;
  if (!userId) {
    throw ApiError.systemError("Missing user context in ownership check", undefined, req.correlationId);
  }
  ...
  logAuthCheck(req, "ownership_or_admin", `owner:${resourceUserId} OR ${adminRole}`, granted);
...
export const requireWardLeader = (wardId: string): ((req: AuthRequest) => void) => {
...
export const requireTreasurer = (scope: string) => requireGroupRole("TREASURER", scope);

export const requireVerifier = (scope?: string) => 
  scope ? requireGroupRole("VERIFIER", scope) : requireSystemRole("VERIFIER");

export const requireModerator = (scope: string) => requireGroupRole("MODERATOR", scope);
 ...
 */
export const toMiddleware = (check: (req: AuthRequest) => void): RequestHandler => {
...
export const hasAnyRole = (req: AuthRequest, roles: string[]): boolean => {
  ...
  return roles.some(role => isValidRole(role) && hasRole(req.user?.roles, role));
};
...
export const hasAllRoles = (req: AuthRequest, roles: string[]): boolean => {
  if (isSuperAdmin(req)) return true;
  return roles.every(role => isValidRole(role) && hasRole(req.user?.roles, role));
};
 ...
 * }
 */
export const userHasRole = (req: AuthRequest, role: string, scope?: string): boolean => {
  if (isSuperAdmin(req)) return true;
  if (!isValidRole(role)) return false;
