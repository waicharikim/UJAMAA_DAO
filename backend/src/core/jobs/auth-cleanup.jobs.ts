...
export function setupAuthCleanupJobs() {
  ...
  cron.schedule('10 3 * * *', async () => {
    try {
      ...
      logger.info(
        { operationType: 'GENERAL' },
        `Cleaned up ${count} verification tokens`
      );
    } catch (error) {
      logger.error(
        { operationType: 'GENERAL', error },
  ...
  cron.schedule('20 3 * * *', async () => {
    try {
      ...
      logger.info(
        { operationType: 'GENERAL' },
        `Cleaned up ${count} phone codes`
      );
    } catch (error) {
      logger.error(
        { operationType: 'GENERAL', error },
        'Failed to cleanup phone codes'
      );
    }
  });
  ...
  cron.schedule('30 3 * * *', async () => {
    try {
      ...
      logger.info(
        { operationType: 'GENERAL' },
        `Cleaned up ${count} reset tokens`
      );
    } catch (error) {
      logger.error(
        { operationType: 'GENERAL', error },
        'Failed to cleanup reset tokens'
      );
    }
  });
  ...
  cron.schedule('0 4 * * 0', async () => {
    try {
      ...
      logger.info(
        { operationType: 'GENERAL' },
        `Cleaned up ${count} old security events`
      );
    } catch (error) {
      logger.error(
        { operationType: 'GENERAL', error },
