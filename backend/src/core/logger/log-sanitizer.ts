...
export function looksLikePhone(str: string): boolean {
  ...
  return (
    /^(\+?254|0)[1-7]\d{8}$/.test(cleaned) || /^\+?[1-9]\d{1,14}$/.test(cleaned)
  );
}
...
export function redactWallet(
  ...
  const cfg = { ...DEFAULT_CONFIG, ...config };
  if (wallet.length < cfg.walletVisibleStart + cfg.walletVisibleEnd)
...
export function redactIP(
  ...
  if (ip.includes('.')) {
    ...
    if (
      parts.length === 4 &&
      parts.every((o) => o.length > 0 && !isNaN(Number(o)))
    ) {
      const visible = parts.slice(0, visibleCount).join('.');
      const masked = Array(4 - visibleCount)
        .fill('xxx')
        .join('.');
...
export function redactPhone(
  ...
  // Generic fallback
  if (cleaned.length < cfg.phoneVisibleStart + cfg.phoneVisibleEnd)
