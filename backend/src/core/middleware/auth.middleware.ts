...
import { AuthRequest, GeographicContext, LocationScope } from "../types/Ujamaadao.types.js";
import { verifyJwtToken, jwtPayloadToAuthUser, JwtPayload } from "../utils/jwt.service.js";
import { ApiError } from "../errors/ApiError.js";
import { logger, logSecurityEvent, createRequestLogger } from "../logger/logger.js";
 ...
 */
async function validateAndPopulateUser(req: AuthRequest, token: string): Promise<void> {
...
export async function authenticate(
  ...
  if (!token) {
    const error = ApiError.authenticationError("Authentication required", undefined, req.correlationId);
  ...
  try {
    ...
    requestLogger.info({ operationType: "AUTH" }, "User authenticated successfully");
    next();
  } catch (error) {
    logSecurityEvent(
    ...
    const apiError = ApiError.tokenInvalid("Invalid or expired token", req.correlationId);
