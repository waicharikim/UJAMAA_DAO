...
import { AuthRequest, UjamaadaoAuthorizeOptions } from "../types/Ujamaadao.types.js";
...
export const authorize = (options: UjamaadaoAuthorizeOptions) => {
  ...
  return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    const correlationId = req.correlationId || `corr-${Date.now()}-${randomUUID()}`;
    const user = req.user;
    ...
    try {
      if (!user || typeof user !== "object") {
        logSecurityEvent(
          ...
          { ipAddress: req.ip, metadata: { path: req.path, method: req.method, correlationId } }
        );
        const error = ApiError.authenticationError("Unauthorized access", undefined, correlationId);
      ...
      if (!Array.isArray(user.roles)) {
        logSecurityEvent(
          ...
          { userId: user.userId, metadata: { path: req.path, method: req.method, correlationId } }
        );
        const error = ApiError.systemError("Invalid user data structure", undefined, correlationId);
        res.status(error.statusCode).json(error.toJSON());
        return;
      }
      ...
      const userLevelIndex = VERIFICATION_LEVELS.indexOf(user.verificationLevel);
      const requiredLevelIndex = VERIFICATION_LEVELS.indexOf(verificationLevel);

      if (userLevelIndex === -1 || requiredLevelIndex === -1) {
        const invalid = userLevelIndex === -1 ? user.verificationLevel : verificationLevel;
        logger.error({ invalidLevel: invalid }, "Invalid verification level detected");
        const error = ApiError.systemError("Invalid verification level configuration", undefined, correlationId);
        res.status(error.statusCode).json(error.toJSON());
        return;
      }
      ...
      if (allowedRoles.length > 0) {
        ...
        const hasRole = hasWildcard || user.roles.some((role) => allowedRoles.includes(role));
        if (!hasRole) {
          logSecurityEvent(
            "Role access denied",
            "ROLE_VIOLATION",
      ...
      if (minImpactPoints !== undefined) {
        ...
        if (current < minImpactPoints) {
          logSecurityEvent(
            ...
            { userId: user.userId, metadata: { required: minImpactPoints, correlationId } }
          );
      ...
      if (minParticipationRights !== undefined) {
        ...
        if (current < minParticipationRights) {
          logSecurityEvent(
            ...
            { userId: user.userId, metadata: { requiredPR: minParticipationRights, correlationId } }
          );
      ...
      if (scopeCheck !== undefined) {
        let allowed: boolean;
        try {
          allowed = typeof scopeCheck === "function"
            ? await Promise.resolve(scopeCheck(req))
            : scopeCheck;
        } catch (err) {
          logger.error({ error: err instanceof Error ? err.message : String(err) }, "Scope check threw");
          const error = ApiError.systemError("Scope validation failed", undefined, correlationId);
          res.status(error.statusCode).json(error.toJSON());
          return;
        }
      ...
      if (resourceOwnerCheck !== undefined) {
        let isOwner: boolean;
        try {
          isOwner = typeof resourceOwnerCheck === "function"
            ? await Promise.resolve(resourceOwnerCheck(req))
            : resourceOwnerCheck;
        } catch (err) {
          logger.error({ error: err instanceof Error ? err.message : String(err) }, "Ownership check threw");
          const error = ApiError.systemError("Ownership validation failed", undefined, correlationId);
          res.status(error.statusCode).json(error.toJSON());
          return;
        }
        ...
        if (!isOwner) {
          ...
          const error = ApiError.authorizationError("Not authorized to access this resource", undefined, correlationId);
          res.status(error.statusCode).json(error.toJSON());
          return;
        }
      }
      ...
      logger.error(
        { userId: user?.userId, error: error instanceof Error ? error.stack : String(error), correlationId },
        "Unexpected error in authorize middleware"
      );

      const apiError = ApiError.systemError("Internal authorization error", undefined, correlationId);
